...
USearchIndexWithSerialization<metric_ak>::USearchIndexWithSerialization(size_t dimensions)
    : Base(Base::make(unum::usearch::metric_punned_t(dimensions, metric_ak)))
{
}
...
template <unum::usearch::metric_kind_t metric_ak>
void USearchIndexWithSerialization<metric_ak>::deserialize([[maybe_unused]] ReadBuffer & istr)
{
...
template <unum::usearch::metric_kind_t metric_ak>
size_t USearchIndexWithSerialization<metric_ak>::getDimensions() const
{
...
template <unum::usearch::metric_kind_t metric_ak>
MergeTreeIndexGranuleUSearch<metric_ak>::MergeTreeIndexGranuleUSearch(const String & index_name_, const Block & index_sample_block_)
    : index_name(index_name_), index_sample_block(index_sample_block_), index(nullptr)
{
}

template <unum::usearch::metric_kind_t metric_ak>
MergeTreeIndexGranuleUSearch<metric_ak>::MergeTreeIndexGranuleUSearch(
    const String & index_name_, const Block & index_sample_block_, USearchIndexWithSerializationPtr<metric_ak> index_)
    : index_name(index_name_), index_sample_block(index_sample_block_), index(std::move(index_))
{
}

template <unum::usearch::metric_kind_t metric_ak>
void MergeTreeIndexGranuleUSearch<metric_ak>::serializeBinary(WriteBuffer & ostr) const
{
...
template <unum::usearch::metric_kind_t metric_ak>
void MergeTreeIndexGranuleUSearch<metric_ak>::deserializeBinary(ReadBuffer & istr, MergeTreeIndexVersion /*version*/)
{
    UInt64 dimension;
    readIntBinary(dimension, istr);
    index = std::make_shared<USearchIndexWithSerialization<metric_ak>>(dimension);
    index->deserialize(istr);
}

template <unum::usearch::metric_kind_t metric_ak>
MergeTreeIndexAggregatorUSearch<metric_ak>::MergeTreeIndexAggregatorUSearch(const String & index_name_, const Block & index_sample_block_)
    : index_name(index_name_), index_sample_block(index_sample_block_)
{
}

template <unum::usearch::metric_kind_t metric_ak>
MergeTreeIndexGranulePtr MergeTreeIndexAggregatorUSearch<metric_ak>::getGranuleAndReset()
{
    auto granule = std::make_shared<MergeTreeIndexGranuleUSearch<metric_ak>>(index_name, index_sample_block, index);
    index = nullptr;
    return granule;
}

template <unum::usearch::metric_kind_t metric_ak>
void MergeTreeIndexAggregatorUSearch<metric_ak>::update(const Block & block, size_t * pos, size_t limit)
{
    if (*pos >= block.rows())
        throw Exception(
    ...
    if (const auto & column_array = typeid_cast<const ColumnArray *>(column_cut.get()))
    {
        ...
        index = std::make_shared<USearchIndexWithSerialization<metric_ak>>(size);
    ...
    else if (const auto & column_tuple = typeid_cast<const ColumnTuple *>(column_cut.get()))
    {
        ...
        index = std::make_shared<USearchIndexWithSerialization<metric_ak>>(data[0].size());
...
std::vector<size_t> MergeTreeIndexConditionUSearch::getUsefulRangesImpl(MergeTreeIndexGranulePtr idx_granule) const
{
    ...
    const auto granule = std::dynamic_pointer_cast<MergeTreeIndexGranuleUSearch<metric_ak>>(idx_granule);
    if (granule == nullptr)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Granule has the wrong type");

    const USearchIndexWithSerializationPtr<metric_ak> index = granule->index;
